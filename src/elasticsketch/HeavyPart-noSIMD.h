...
public:
    ...
    int insert(const void *key, uint8_t *swap_key, uint32_t &swap_val, uint32_t f = 1)
    {
        ...
        uint32_t guard_val = buckets[pos].val[MAX_VALID_COUNTER-1];
        guard_val = UPDATE_GUARD_VAL(guard_val);

        if (!JUDGE_IF_SWAP(GetCounterVal(min_counter_val), guard_val))
        {
            buckets[pos].val[MAX_VALID_COUNTER-1] = guard_val;
            return 2;
        }
        ...
        *((uint32_t *)swap_key) = buckets[pos].key[min_counter];
        ...
        buckets[pos].val[MAX_VALID_COUNTER-1] = 0;
    ...
    uint32_t query(uint8_t *key)
    {
        ...
        for (int i = 0; i < MAX_VALID_COUNTER-1; ++i)
            if (buckets[pos].key[i] == fp)
