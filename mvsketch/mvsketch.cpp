...
void MVSketch::Update(unsigned char* key, val_tp val) {
    ...
    for (int i = 0; i < mv_.depth; i++) {
        bucket = MurmurHash3_x86_32(key, keylen, i * i) % mv_.width;
...
typedef struct {
    ...
    long operator() (const key_tp &k) const { return MurmurHash3_x86_32(&k, LGN, 388650253); }
} key_tp_hash;
...
void MVSketch::Query_all(val_tp thresh, std::vector<std::pair<key_tp, val_tp> >&results) {
    ...
    for (auto it = res.begin(); it != res.end(); it++) {
        ...
        for (int j = 0; j < mv_.depth; j++) {
            unsigned long bucket = MurmurHash3_x86_32((void*)(& *it), mv_.lgn/8, j * j) % mv_.width;
...
//     int min
//     for (int j = 0; j < mv_.depth; j++) {
//         unsigned long bucket = MurmurHash3_x86_32(key, mv_.lgn/8, j * j) % mv_.width;
//         unsigned long index = j*mv_.width+bucket;
...
val_tp MVSketch::Query_fast(key_tp* key) {
    val_tp ret = 0;
    for (int i = 0; i < mv_.depth; i++) {
        unsigned long bucket = MurmurHash3_x86_32(key, mv_.lgn/8, i * i) % mv_.width;
